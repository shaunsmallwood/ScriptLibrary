# Uncomment and customize if not already connected to MSOL/EXO
# Connect-MsolService
...
$tenantaccountname = ((Get-MsolAccountSku).AccountName | Select-Object -First 1)
$mailboxes = Get-EXOMailbox -ResultSize unlimited -Properties ArchiveStatus,RetentionPolicy,AutoExpandingArchiveEnabled
...
foreach($mailbox in $mailboxes) {
    ...
    if ($MSOLAccount.IsLicensed -eq "TRUE") {
         $MSLicenses = (($MSOLAccount.Licenses.AccountSkuID -replace ("$tenantaccountname"+":"),"" | ForEach-Object { ($MSProdArray -match "\b$_\b").ProductName }) -Join " | ").ToUpper()
    } else {
         $MSLicenses = ""
    }

    $ExportObject = [PSCustomObject]@{
        "Display Name" = $MSOLAccount.DisplayName
        "Username (MSOL)" = $MSOLAccount.UserPrincipalName
        ...
        "Is Licensed" = $MSOLAccount.IsLicensed
        "Licenses Assigned" = "$MSLicenses"
        "Login Blocked" = $MSOLAccount.BlockCredential
